...
__version__ = '$Id: collision.py 565 2009-09-06 12:30:41Z DLacewell $'
...
class BoundingRadiusStrategy(ICollisionStrategy):
    ...
    def _dummy_func(self, a, b):
        # TODO: probably raise an exception since this collision is not registered
        if __debug__: print '_dummy_func(a, b): \n\ta=%s of type %s \n\tb=%s of type %s' % (a, a.__class__, b, b.__class__)
...
def brute_force(objects1, objects2, collide_func): # -> [(obj, obj), ()] list of colliding pairs
    ...
    if objects1 == objects2:
        ...
        return [(entity, objects2[idx]) \
                    for entity in objects1 \
                    for idx, other in enumerate(objects2) if collide_func(entity, other)]
...
def brute_force_radius(objects1, objects2):
    ...
    if objects1 == objects2:
        ...
        return [(entity, objects2[idx]) \
                    for entity in objects1 \
                    for idx, other in enumerate(objects2) if (entity.position - other.position).lengthSQ < (entity.bounding_radius + other.bounding_radius) * (entity.bounding_radius + other.bounding_radius)]
...
if __name__ == '__main__':
    ...
    class A(object):
        ...
        def collision_response(self, *args):
            if __debug__: print 'response', args
        def update(self):
...
#    d.check_narrow(pairs)
    ...
    while running:
        ...
        if __debug__: print 'brute_force_rect', delta

        start = time.time()
        c = rdc_rect(g)
        delta = time.time() - start
        if __debug__: print 'rdc_rect        ', delta, '\n'

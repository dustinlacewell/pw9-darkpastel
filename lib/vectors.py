...
__version__ = '$Id: vectors.py 565 2009-09-06 12:30:41Z DLacewell $'
...
#        return u"<%s(%s, %s)>" %(self.__class__.__name__, self.x, self.y)
    ...
    def get_distanceSQ(self, other):
        u"""
        Distance squared this and other point (represented as vector from origin).
...
class Vec3(object):
    ...
    length = property(_length, _set_length, doc=u"""get/set the length of vector""")
...
#        return u"<%s(%s, %s, %s)>" %(self.__class__.__name__, self.x, self.y, self.z)
    ...
    def get_full_angle_between(self, other):
        ...
        olen = other.length
        if length and olen:
            return  degrees(atan2(other.y, other.x) - atan2(self.y, self.x))
        return 0

    def get_distanceSQ(self, other):
        u"""
        Distance squared from this and other point (represented as vector from origin).
...
if __name__ == '__main__':
    ...
    v = Vec(1,0)
    w = Vec(0,1)
    assert v.get_angle_between(w) == 90
    assert w.get_angle_between(v) == 90
    w = Vec(1,1)
    assert round(w.get_angle_between(v)) == 45.0
    assert round(v.get_angle_between(w)) == 45.0
...
#    print w + v
    v = Vec3(1,1)
    w = Vec3(1,10)
    print 'between v w', v.get_angle_between(w)
    print 'between w v', w.get_angle_between(v)
    print 'v.angle', v.angle, 'w.angle', w.angle
    print 'angle sub v w', v.angle - w.angle
    print 'angle sub w v', w.angle - v.angle
    
    raw_input(u'press any key to continue...')
cerealizer.register(Vec3)
